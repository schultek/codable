...
abstract interface class Encodable<T> {
  ...
  /// ```
  static Encodable<T> self<T extends SelfEncodable>() => _EncodableSelf<T>();
...
///   }
/// }
abstract class SelfCodable<T extends SelfEncodable> extends _EncodableSelf<T> implements Codable<T> {
  const SelfCodable();
...
class _EncodableSelf<T extends SelfEncodable> implements Encodable<T> {
  const _EncodableSelf();
